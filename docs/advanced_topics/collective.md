...
!!! warning
    A two server collective is actually not redundant, as a minimum of 3 servers is required for redundancy.

Any member of a collective can invite an additional member.  For example, extending the collective above
to include `mgt3` could be done from `mgt1`, but we will do it from `mgt2` in this case:
