...
If no network booting or [node discovery](../user_reference/confluentdiscovery.md) using switch port information for node identification is required, disabling the setting can be done to improve the performance of Confluent.

The setting must be defined for each Confluent management node, including all members of the [collective](collective.md) if collective mode is in use.
