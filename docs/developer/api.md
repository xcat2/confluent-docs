...
*Quick summary should go here maybe*
...
* **health** - An assessment of whether the general state of the node should <!-- is this right or is this just the health of the component? -->
...
about the node by confluent, but are not directly active on the system.    <!-- I'm not sure what this means -->
...
PUT {'state': 'reset'} in order to initiate a restart of the management    <!-- Why do you need to PUT reset when it's /reset? -->
controller
...
#### Configuring NTP: /nodes/[nodename]/configuration/management_controller/ntp/   

Configure and control the NTP functionality of supported management controllers.  <!-- Replace with two sub commands? -->
...
the respective destination to more destinations and formats.  Each item contains   <!-- In general... Specify read/write -->
...
### Opening a console session: /nodes/[nodename]/console/session
...
### Viewing and setting node attributes: /nodes/[nodename]/attributes/
...
A list of devices that are possible and whether they are present and associated   <!-- what is category? name, info, etc? -->
data.  The inventory field is a list of objects with the following fields:    <!-- what is inventory field? -->
...
* **component** - A textual description of a physical or logical entity related to
              the event.
* **component_type** - A description of what sort of entity the component is   <!-- Examples? -->
...
* **id** - A numerical value representing the id, useful for looking up the id
       against a database
* **record_id** - An identifier associated with the event by the providing device  <!-- used for what? -->
