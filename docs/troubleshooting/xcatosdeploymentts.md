...
<https://sourceforge.net/p/xcat/wiki/Building_a_Stateless_Image_of_a_Different_Architecture_or_OS_Old/>
...
!!! note
    This is due to the xCAT `genimage` application failing to set the `sshd` service to start automatically during stateless image creation of SLES 15.2.  As a result, the image is not reachable via SSH after deployment.
