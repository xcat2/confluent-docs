...
```console
# nodeshell bf3 "apt-get install net-tools" | collate
...
Modifying the `nodeshell` command to have the DEBIAN_FRONTEND set to noninteractive will fix this issue

```bash
nodeshell bf3 "DEBIAN_FRONTEND=noninteractive apt-get -y install net-tools" | collate
```
