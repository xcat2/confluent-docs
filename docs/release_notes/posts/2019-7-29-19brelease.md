...
`Administrator` is the usual full access, `Operator` is blocked from modifying users/passwords.  `Monitor` is severely limited to select read-only access appropriate for use in monitoring scripts.  This may be done to users or groups.
