...
As well as refreshing the general level of software contained in genesis, some key enhancements have been implemented:

* Confluent genesis may now be targeted by ansible plays
...
A collective may specify which managers are permitted to scan a switch. This can be used to speed up switch interrogation
...
* Some environment variables have had 'confluent_' prepended, to avoid conflict with other uses of the common variable names.
